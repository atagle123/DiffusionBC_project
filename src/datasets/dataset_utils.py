...
from src.datasets.normalization import (
    BaseNormalizer,
    GaussianNormalizer,
    MinMaxNormalizer,
)
...
def sequence_dataset(env, dataset=None, **kwargs):
    ...
    for i in range(N):
        ...
        for k in [
            "observations",
            "actions",
        ]:  # for compatibility with v2 environments. only use observation and actions
...
class EpisodeDataset:
    ...
    def preprocess(
        ...
        fields_to_normalize: list[str] = ["actions", "observations"],
    ):
        self.normalize_fields(
            fields_to_normalize=fields_to_normalize, normalization=normalization
    ...
    def _get_normalization_params(self, fields_to_normalize: list[str]) -> dict:
        norm_params = {}
    ...
    def _get_normalization_class(self, normalization: str) -> BaseNormalizer:
        """
    ...
    def shift_actions(self, pad_val: float = 0.0):
        ...
        for episode in self.episodes:
            ...
            episode["actions"][0, :] = (
                pad_val  # zero padding at the beggining of actions
            )
    ...
    def pad(
        ...
        history_len: int,
        pad_val: int = 0,
        pad_fields: list[str] = ["actions", "observations"],
    ):
        assert history_len >= 1
        for episode_length, episode in zip(self.episodes_length, self.episodes):
            for field in pad_fields:
                episode[field] = np.pad(
                    episode[field],
                    pad_width=((history_len - 1, 0), (0, 0)),
