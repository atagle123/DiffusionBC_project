...
class MaskedDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        p_mask_set: list = [0.15, 0.35, 0.55, 0.75, 0.95],
        mask_strategy: str = "all",
        unmask_start: bool = False,
        mask_end: bool = False,
        **kwargs,
    ):
    ...
    def make_masks(self):
        ...
        n_masked = (
            np.random.choice(self.p_mask_set, self.mask_cache_size) * self.horizon
        )
        n_masked = n_masked.astype(np.int_)

        if self.mask_strategy == "all":
            self.masks = np.zeros(
                (self.horizon, self.action_dim + self.state_dim), dtype=bool
            )
            ...
            self.mask_strategy == "random_split"
        ):  # random split between history and future
            self.masks = np.zeros(
                (self.mask_cache_size, self.horizon, self.action_dim + self.state_dim),
                dtype=bool,
            )
            n_masked = np.random.randint(
                self.horizon, size=self.mask_cache_size
            )  # allow possibility of masking everything
            ...
            self.mask_strategy == "indep_state_action"
        ):  # masking independent at level of state and action only
            mask_a = np.ones(
                (self.mask_cache_size, self.horizon, self.action_dim), dtype=bool
            )
            mask_s = np.ones(
                (self.mask_cache_size, self.horizon, self.state_dim), dtype=bool
            )
        ...
        if self.mask_end and self.mask_strategy not in ["all", "halfrandom_split"]:
            self.masks[:, -1, :] = False
