...
class InpaintHistoryBuffer(HistoryBuffer): 
    def __init__(self, normalizer, history_len: int, action_dim: int, obs_dim: int, batch_size: int, pad_value: float = 0.0, device=DEVICE):
        super().__init__(normalizer, history_len, action_dim, obs_dim, batch_size, pad_value, device)
...
class Inpainting_Agent(FiLM_Agent):
    ...
    def policy(self, state): # TODO... 
        ...
        actions = samples[:,self.history_buffer.len, :self.action_dim].cpu().numpy() # action that follows curr history
