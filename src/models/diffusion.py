...
class GaussianDiffusionRePaint(GaussianDiffusion):
    """
    TODO: add repaint sampling
    """
    ...
    def p_sample_loop(self, shape: tuple[int]) -> Sample:
        """
        Classical DDPM sampling algorithm
    ...
    @torch.no_grad()
    def forward(self, batch_size: int, mask, known_data) -> Sample:
        """ """

        return self.p_sample_loop(shape=(batch_size, self.data_dim))
